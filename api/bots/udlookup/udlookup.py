...
HELP_MESSAGE = '''
            This bot allows users to look up the definitions and meanings
            of acronyms, internet-speak, and slang words.
            ...
            The 'yoda.config' file should be located at '~/udlookup.config'.
            Example input:
...
class UDLookUpHandler(object):
    '''
    This bot will allow users to look up the definitions and meanings
    of acronyms, internet-speak, and slang words.
    It looks for messages starting with '@mention-bot'.
    '''

    def initialize(self, bot_handler):
        self.api_key = bot_handler.get_config_info('udlookup')['api_key']

    def usage(self):
        return '''
            This bot will allow users to ook up the definitions and
            meanings of acronyms, internet-speak, and slang words.
            Users should preface messages with '@mention-bot'.

            Before running this, make sure to get a Mashape Api token.
            Instructions are in the 'readme.md' file.
            Store it in the 'udlookup.conf' file.
            The 'udlookup.conf' file should be located in this bot's directory.
            Example input:
            @mention-bot You will learn how to speak like me someday.
            '''
...
def send_to_udlookup_api(term, api_key):
    # function for sending sentence to api

    response = requests.get("GEThttps://mashape-community-urban-dictionary.p.mashape.com/define?"/
                            + term,
                            headers={
...
def handle_input(message, bot_handler):
    ...
    if is_help(original_content):
        ...
        try:
            reply_message = send_to_udlookup_api(phrase, handler_class.api_key)
